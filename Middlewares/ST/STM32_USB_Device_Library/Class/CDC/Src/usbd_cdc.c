...
0x01, /* bNumEndpoints: One endpoints used */
0x02, /* bInterfaceClass: Communication Interface Class */
0x06, /* bInterfaceSubClass: Ethernet Networking Control Model*/
0x00, /* bInterfaceProtocol: No Protocol Specified */
0x00, /* iInterface: */
...
//0x01,   /* bDataInterface: 1 */
		...
		13, /* bLength */
		0x24, /* bDescriptor Type: 0x24 CS_INTERFACE */
		0x0F, /* bDescriptorSubtype: Ethernet Netoworking Interface */
		USBD_IDX_MAC_STR, /* iMacAddress: index of string descriptor of MAC address */
		0x00, /* bmEthernetStatistics */
		0x00, 0x00, 0x00, 0xEA, /* wMaxSegmentSize */
		0x05, 0x00, 0x00, /* wNumberMCFilters */
...
static uint8_t USBD_CDC_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx) {
	...
	if (pdev->pClassData == NULL) {
		...
		if (pdev->dev_speed == USBD_SPEED_HIGH) {
			...
			CDC_OUT_EP, hcdc->RxBuffer, hcdc->RxLength); //CDC_DATA_HS_OUT_PACKET_SIZE);
		} else {
			/* Prepare Out endpoint to receive next packet */
			USBD_LL_PrepareReceive(pdev,
			CDC_OUT_EP, hcdc->RxBuffer, hcdc->RxLength); //CDC_DATA_FS_OUT_PACKET_SIZE);
		}
...
static uint8_t USBD_CDC_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum) {
	USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef*) pdev->pClassData;
	printf("Tx end Irq %d y num %x\n\r", hcdc->TxState,epnum);
	if (pdev->pClassData != NULL) {

		if (epnum == (CDC_CMD_EP & 0b00001111)) {
			printf("irq tx notify sended\n\r");
			hcdc->NtState = 0;
...
static uint8_t USBD_CDC_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum) {
	...
	hcdc->RxLength -=
			((PCD_HandleTypeDef *) pdev->pData)->OUT_ep[epnum].xfer_len
					- ((PCD_HandleTypeDef *) pdev->pData)->OUT_ep[epnum].xfer_count;
...
/*uint8_t USBD_CDC_Notify(USBD_HandleTypeDef *pdev) {
	USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef*) pdev->pClassData;
	...
	if (pdev->pClassData != NULL) {
		if (hcdc->NtState == 0) {
			...
			USBD_LL_Transmit(pdev,
			CDC_CMD_EP, hcdc->NotificationBuffer, hcdc->NtLength);
...
uint8_t USBD_CDC_Notify(USBD_HandleTypeDef *pdev){
	...
	if (pdev->pClassData != NULL) {
		if (hcdc->NtState == 0) {
			...
			hcdc->NtState = 1;
			USBD_LL_Transmit(pdev,
			CDC_CMD_EP, hcdc->NtBuffer, hcdc->NtLength);
...
/* added by user */
uint8_t USBD_CDC_ReceiveData(USBD_HandleTypeDef *pdev) {
	USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef*) pdev->pClassData;
	...
	if (pdev->pClassData != NULL) {
		if (pdev->dev_speed == USBD_SPEED_HIGH) {
			/* Prepare Out endpoint to receive next packet */
			USBD_LL_PrepareReceive(pdev,
			CDC_OUT_EP, hcdc->RxBuffer, hcdc->RxLength);
		} else {
			/* Prepare Out endpoint to receive next packet */
			USBD_LL_PrepareReceive(pdev,
			CDC_OUT_EP, hcdc->RxBuffer, hcdc->RxLength);
